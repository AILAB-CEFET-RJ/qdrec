...
def get_unique_named_entity(db: Session, named_entity: schemas.NamedEntity):
    return db.query(excerpt_models.NamedEntity).filter(excerpt_models.NamedEntity.content == named_entity.content).filter(excerpt_models.NamedEntity.excerpt_id == named_entity.excerpt_id).filter(excerpt_models.NamedEntity.entity_type == named_entity.entity_type).first()